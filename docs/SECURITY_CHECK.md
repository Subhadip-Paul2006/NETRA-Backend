...
### 7.2 Findings Pipeline Security, Integrity & Failure Isolation (Phase 8.7.5)

* **Exact Fingerprint Determinism**: Finding deduplication fingerprints are computed canonically as `SHA-256(rule_id || ":" || target_key || ":" || raw_discriminator)` and persisted atomically into `local_findings` without discriminator aliasing.
* **Failure-Isolated Reconciliation**: Telemetry degradation, scanner timeout, permission denial, or missing collector sources are strictly isolated. Automated finding resolution occurs exclusively when source telemetry is authoritatively present and clean. Failed scans never falsely resolve existing security findings.
* **Transaction Isolation**: Transaction A (raw observation queue and findings persistence) and Transaction B (derived topology observation enqueue) are strictly decoupled. A persistence fault in Transaction B cannot roll back or corrupt Transaction A records.
* **Zero Speculative Attribution**: Findings generated by network posture rules (`NET-003` through `NET-008`) report factual configuration states against RFC standards without fabricating threat actor claims or unauthorized assumptions.
