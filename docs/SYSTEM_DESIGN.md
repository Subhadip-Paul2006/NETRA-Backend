...
6. **`IN_MEMORY_TOPOLOGY_SYNTHESIS`**: Deterministic in-memory graph synthesis inside `ScannerSupervisor` (`TopologyCorrelator` + `TopologyExtractor` in `netra-core`). Derives typed correlation edges (`InterfaceHostsSubnet`, `InterfaceHasGateway`, `InterfaceHasNeighbor`, `NeighborIsGateway`, `GatewayOnSubnet`, `DnsOnSubnet`) in pure memory (zero I/O). The resulting derived topology observation is evaluated through `RuleEngine::with_all_rules()` in a single unified pass alongside raw observations, and persisted in a failure-isolated SQLite transaction (Transaction B).
